...
def gg():
    ...
    for edge in edge_list:
        ...
        if count < len(edge_list):
            print(str('<') + str(v1) + str(',') + str(v2) + str('>') + str(','),end = '')
        else:
            print(str('<') + str(v1) + str(',') + str(v2) + str('>'),end = '')
    print('}')
    clear()
...
def main():
    ...
    while True:
        ...
        a=r'^(add)\s\"([a-zA-Z0-9]+\s?)*([a-zA-Z0-9]+)\"\s(\(\-?\d+\,\-?\d+\)\s)+\(\-?\d+\,\-?\d+\)$'
        add_check = re.compile(a)
        m=r'^(mod)\s\"([a-zA-Z0-9]+\s?)*([a-zA-Z0-9]+)\"\s(\(\-?\d+\,\-?\d+\)\s)+\(\-?\d+\,\-?\d+\)$'
        mod_check = re.compile(m)
        r= r'^(rm)\s\"([a-zA-Z0-9]+\s?)*([a-zA-Z0-9]+)\"$'
